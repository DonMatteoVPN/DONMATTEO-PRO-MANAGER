...
# =============================================================================
# СОЗДАНИЕ СТРУКТУРЫ ПАПОК И ОЧИСТКА СТАРЫХ ВЕРСИЙ
# =============================================================================
echo -e "${CYAN}[1/5] Подготовка директорий и очистка старых версий...${NC}"
mkdir -p "${BASE_DIR}" "${CORE_DIR}" "${CONF_DIR}" "${MOD_DIR}"

# Удаляем старые модули в корне modules/ (до рефакторинга), чтобы избежать дублей
rm -f "${MOD_DIR}"/m_*.sh 2>/dev/null || true
# Удаляем старый загрузчик don
rm -f "${BASE_DIR}/don" 2>/dev/null || true
