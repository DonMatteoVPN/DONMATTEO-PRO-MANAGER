...
install_menu() {
    while true; do
        ...
        echo -e "${BLUE}======================================================${NC}"
        echo -e " ${CYAN}Перед установкой убедитесь, что Nginx и Xray${NC}"
        echo -e " ${CYAN}настроены на передачу IP-адресов клиентов.${NC}"
        echo -e " ${YELLOW}👉 Нажмите [6], чтобы прочитать инструкцию.${NC}"
        echo -e "${BLUE}======================================================${NC}"
        echo -e " ${GREEN}${BOLD}1.${NC} 🚀 ${BOLD}УСТАНОВИТЬ ВСЁ СРАЗУ${NC} ${GRAY}(рекомендуется)${NC}"
        echo -e "${GRAY}    ├─ ⚡ Защита ядра (Sysctl/BBR/Anti-DDoS)${NC}"
        echo -e "${GRAY}    ├─ 🌐 Сетевой экран (UFW + лимиты портов)${NC}"
        echo -e "${GRAY}    ├─ 👮 Бан брутфорсеров (Fail2Ban + фильтры)${NC}"
        echo -e "${GRAY}    └─ 📂 Ротация логов (Logrotate)${NC}"
        echo -e "${BLUE}------------------------------------------------------${NC}"
        echo -e " ${YELLOW}2.${NC} ⚡ Только защита ядра (Sysctl/BBR)       $(get_sysctl_status)"
        echo -e " ${YELLOW}3.${NC} 🌐 Только инициализация UFW               $(get_ufw_status)"
        echo -e " ${YELLOW}4.${NC} 👮 Только установка Fail2Ban (бан-листы)  $(get_f2b_status)"
        echo -e " ${YELLOW}5.${NC} 📂 Только настройка ротации логов         $(get_logrotate_status)"
        echo -e "${BLUE}------------------------------------------------------${NC}"
        echo -e " ${MAGENTA}${BOLD}6. 📖 ЧИТАТЬ ИНСТРУКЦИЮ (ИДЕАЛЬНЫЙ NGINX + XRAY)${NC}"
